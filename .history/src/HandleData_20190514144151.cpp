...
int Csv2DbfCommand(string strFilePath)
{
    ...
    */
    ...
    int i = 0;
    char* token;
    std::string surround = strIncAndSplit.substr(0, 1);
    const char * const delim = strIncAndSplit.substr(1, 1).c_str();
    ...
    while(getline(csv, buff))
    {
        ...
        for (token = strsep(&oristr, delim), i = 0; token != NULL, i < iColumns; token = strsep(&oristr, delim), i++)
        {
            ...
            if (surround != "0")
            {
                trim(szstrTmp[i], surround); // 从gdb看导致delim改变了，不应该啊，delim是const
